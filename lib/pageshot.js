...
async function capture ({ client, viewportHeight, viewportWidth, full, delayTime }) {
  ...
  fs.writeFileSync('wtf2.png', Buffer.from(data, 'base64'))
  //console.log(data);

  var wtf = await Jimp.read(Buffer.from(data, 'base64')).then( function(wtf) {
    wtf.resize(500,500)
    //wtf.write('resized.png')
    return wtf
  }).catch (function(err) {
    console.error(err);
  });
...
/*
  var buf = await wtf.getBase64(Jimp.MIME_PNG, ).then(function(b64) {
    var trimmed = b64.slice(22);
...
*/
  ...
  return wtf;//Buffer.from(data, 'base64');
